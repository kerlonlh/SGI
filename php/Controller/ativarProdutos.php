...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id'])) {
        ...
        if ($linha) {
            ?>
                <section>
                    <h1>Ativar produtos</h1>
                    <form action="index.php?pg=resultado-ativacao-produtos" method="POST">
                        <?php
                             echo '<div class="message__error">Tem certeza que deseja ativar o produto ' . htmlspecialchars($linha['produto']) . ', marca ' . htmlspecialchars($linha['marca']) . '?</div>';
                        ?>
                        <div>
