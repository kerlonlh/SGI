...
<body>
    <section>
        ...
        if (isset($_POST['id'])) {
            ...
            $sql_update = "UPDATE produtos SET situacao = 1 WHERE id = :id";
            $sql_update = $pdo->prepare($sql_update);

            if ($sql_update->execute([':id' => $id])) {
                echo '<div class="message__success">' . htmlspecialchars($produto) . ' inativado com sucesso!</div>';
            } else {
                echo '<div class="message__error">Erro ao inativar o produto ' . htmlspecialchars($produto) . '.</div>';
            }
        } 
        ?>
    </section>
